...
<form method="post" action="<?=@route('id=' . $configuration->id)?>"
	class="-koowa-form" id="mainform"><label for="title" class="mainlabel"><?=@text('WXPARAMS_TITLE');?></label>
...
<?=@helper('com://admin/wxparams.template.helper.listbox.' . JVersion::isCompatible('1.6.0') ? 'j16' : 'j15' . '.menuitems');?><br />
<?
}
?>
<? // Render the XML form
